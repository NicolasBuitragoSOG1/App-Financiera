<template>
  <div class="p-6">
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-gray-900 dark:text-white">AI Financial Assistant</h1>
      <p class="text-gray-600 dark:text-gray-400">Get personalized financial advice and recommendations</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="card h-96 flex flex-col">
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-4" ref="chatContainer">
            <div v-if="aiStore.chatHistory.length === 0" class="text-center py-8">
              ...
              <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">Welcome to your AI Financial Assistant</h3>
              <p class="text-gray-500 dark:text-gray-400 mb-4">Ask me anything about your finances, budgeting, or investment strategies.</p>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-2 max-w-md mx-auto">
                <button
                  v-for="suggestion in quickSuggestions"
                  :key="suggestion"
                  @click="askQuestion(suggestion)"
                  class="text-sm bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 px-3 py-2 rounded-lg transition-colors"
                >
                  {{ suggestion }}
                </button>
              </div>
            ...
            <div v-if="aiStore.isLoading" class="flex justify-start">
              <div class="bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white max-w-xs lg:max-w-md px-4 py-2 rounded-lg">
      ...
      <div class="space-y-6">
        <!-- Financial Overview -->
        <div class="card">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Your Financial Snapshot</h3>
          <div class="space-y-3">
            <div class="flex justify-between">
              <span class="text-sm text-gray-600 dark:text-gray-400">Total Balance</span>
              <span class="text-sm font-medium text-gray-900 dark:text-white">${{ formatCurrency(financeStore.totalBalance) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-sm text-gray-600 dark:text-gray-400">Monthly Income</span>
              <span class="text-sm font-medium text-green-600">
                ${{ formatCurrency(financeStore.overview?.monthly_income || 0) }}
              </span>
            </div>
            <div class="flex justify-between">
              <span class="text-sm text-gray-600 dark:text-gray-400">Monthly Expenses</span>
            ...
            <div class="flex justify-between">
              ...
              <span class="text-sm font-medium text-gray-900 dark:text-white">{{ financeStore.goals.length }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="space-y-2">
            ...
            >
              <div class="flex items-center">
                <component :is="action.icon" class="w-5 h-5 text-primary-600 dark:text-primary-400 mr-3" />
                <div>
                  <p class="text-sm font-medium text-gray-900 dark:text-white">{{ action.text }}</p>
        ...
        <!-- AI Tips -->
        <div class="card">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">AI Tips</h3>
          <div class="space-y-3">
            <div v-for="tip in aiTips" :key="tip.title" class="p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
              <h4 class="text-sm font-medium text-blue-900 dark:text-blue-300">{{ tip.title }}</h4>
              <p class="text-xs text-blue-700 dark:text-blue-400 mt-1">{{ tip.description }}</p>
            </div>
          </div>
        </div>
