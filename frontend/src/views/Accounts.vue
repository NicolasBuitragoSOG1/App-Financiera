  ...
  <div class="p-6">
    <div class="flex justify-between items-center mb-8">
      <div>
        ...
        <p class="text-gray-600">Manage your financial accounts across different platforms</p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        ...
        <div class="space-y-2">
          <div class="flex justify-between">
            ...
            <span class="text-sm font-medium capitalize">{{ account.account_type }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-sm text-gray-600">Account Number</span>
            <span class="text-sm font-mono">****{{ account.account_number.slice(-4) }}</span>
          </div>
          <div class="flex justify-between">
      ...
      <div v-if="financeStore.accounts.length === 0" class="col-span-full">
        <div class="text-center py-12">
          ...
          <p class="text-gray-500 mb-6">Get started by adding your first financial account</p>
          <button @click="showCreateModal = true" class="btn-primary">
    ...
    <div v-if="showUpdateModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl p-6 w-full max-w-md mx-4">
        ...
        <div class="mb-4">
          <p class="text-sm text-gray-600">{{ selectedAccount?.account_name }}</p>
          <p class="text-lg font-semibold">Current: ${{ formatCurrency(selectedAccount?.current_balance || 0) }}</p>
        </div>
...
const editAccount = (account) => {
  ...
  toast.info('Account editing coming soon!')
}
</script>
